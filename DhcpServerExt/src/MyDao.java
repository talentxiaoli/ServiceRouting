...
public class MyDao extends BaseDao{
	...
	public List<USModel> queryAll(){
		String sql="SELECT * FROM `url_sid_conflict_list`";
	...
	public List<USModel> queryByUrl(String url) {
		String sql="SELECT * FROM `url_sid_conflict_list` where url = ?";
		List<USModel> list =new ArrayList<USModel>();
		Connection conn=this.getconn();
		PreparedStatement pst=null;
		ResultSet rs=null;
	...
	public int insert(USModel t){
		String str="INSERT INTO `url_sid_conflict_list`(id, url, service_id) VALUE(?,?,?)";
		return executeUpdate(str, new Object[]{t.getId(), t.getUrl(), t.getService_id()});
	}
	
	//修改方法
	public int update(USModel r){
		String sql="UPDATE `url_sid_conflict_list` SET `url` = ?,`service_id` = ?, WHERE service_id = ?";
		return executeUpdate(sql, new Object[]{r.getId(), r.getUrl(), r.getService_id()});
	}
	
	//删除方法
	public int delete(USModel e){
		String sql="DELETE FROM `url_sid_conflict_list` WHERE service_id = ?";
